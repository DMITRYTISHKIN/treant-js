...
;( function() {
	...
	TreeNode.prototype.buildNodeFromTemplate = function(node, template, viewRef) {
		var view = viewRef.createEmbeddedView(template, { data: this.nodeStructure })
		view.detectChanges();
		node.appendChild(view.rootNodes[0]);
		return node;
	}
	...
	TreeNode.prototype.createGeometry = function( tree ) {
			...
			if ( !this.pseudo ) {
					...
					if (tree.CONFIG.nodeTemplate) {
						node = this.buildNodeFromTemplate(node, tree.CONFIG.nodeTemplate, tree.CONFIG.viewContainerRef);
					} else if (this.nodeInnerHTML) {
						node = this.buildNodeFromText(node)
					} else {
						node = this.buildNodeFromText(node);
					}
